...
// CORREÇÃO 1: Interface flexível segura (sem any)
interface HabitoFlexivel {
  nome?: string;
  name?: string;
  titulo?: string;
  descricao?: string;
  // Permite outras propriedades desconhecidas sem usar 'any'
  [key: string]: unknown; 
}
...
const resolveHabitName = (h: HabitoFlexivel | null | undefined | unknown): string => {
  if (!h || typeof h !== 'object') return "Tarefa sem nome";
  
  const habito = h as HabitoFlexivel;
  return habito.nome || habito.name || habito.titulo || "Sem Nome";
};
...
export function Dashboard() {
  ...
  useEffect(() => {
    const fetchAgendaDoDia = async () => {
      ...
      try {
        ...
        const dadosNormalizados = response.data.map((ag: Record<string, unknown>) => ({
          ...ag,
          habito: {
            ...(ag.habito as Record<string, unknown>),
            nome: resolveHabitName(ag.habito) 
          }
        }));

        setAgendamentos(dadosNormalizados as Agendamento[]);
    ...
    const fetchDadosDaSemana = async () => {
      ...
      try {
        ...
        response.data.forEach((molde: MoldeEstatistica) => { 
          const nome = resolveHabitName(molde as unknown as HabitoFlexivel);
  ...
  const handleToggleTarefa = async (agendamento: Agendamento, isChecked: boolean) => {
    ...
    try {
      ...
      setAgendamentos(prev => 
        prev.map(a => a.id === agendamento.id ? { ...a, status: agendamento.status } : a)
      );
    ...
    setAgendamentos(prev =>
      prev.map(ag => 
        ag.habito.id === habitoAtualizado.id
          ? { ...ag, habito: { ...ag.habito, ...habitoAtualizado } } 
          : ag
      )
    );
  };
