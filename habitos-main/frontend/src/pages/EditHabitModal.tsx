...
interface EditModalProps {
  habitoMolde: HabitoMold; // O molde que está sendo editado
  onClose: () => void; // Função para fechar o modal
  onHabitUpdated: (habitoAtualizado: HabitoMold) => void; // Função para atualizar a lista
}
...
export function EditHabitModal({ habitoMolde, onClose, onHabitUpdated }: EditModalProps) {
  ...
  // Garantir que o formulário atualize se o 'habito' (prop) mudar
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Chama o endpoint PUT /api/habitos/:id
      const response = await api.put(`/habitos/${habitoMolde.id}`, {
        nome,
        descricao,
      });

      // Se deu certo (200), avisa o Dashboard
  ...
  return (
    // O 'modal-backdrop' é o fundo escuro
    <div className="modal-backdrop" onClick={onClose}>
      {/* O 'modal-content' é o pop-up branco */}
