...
// --- Tipos Adaptados ao seu Dashboard ---
...
interface AgendaItem {
  id: number;      // ID do agendamento (ex: 15)
  data: string;    
  status: string;  
...
export function FocusMode() {
  ...
  const fetchHabits = async () => {
    try {
      ...
      // FILTRO: Apenas itens pendentes
      // Normalizamos os dados para garantir que habito.nome exista
  ...
  // 2. Concluir Tarefa (USANDO A ROTA DO DASHBOARD)
  const markAsDone = async () => {
    ...
    try {
      ...
      // --- CORREÇÃO FINAL: Mudei para PUT (igual ao Dashboard) ---
          ...
          {items.map(item => (
            <div key={item.id} className="focus-card">
              <div className="focus-card-content">
                {/* Nome garantido pelo map no fetch */}
