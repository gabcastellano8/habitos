...
export function Estatisticas() {
  ...
  const timeline = useMemo<TimelineData>(() => {
    ...
    // Definimos 'today' para o final absoluto do dia (23:59:59)
    // Assim, 'current' (que é 12:00) será sempre menor que 'today', incluindo o dia atual.
  ...
  // --- STATUS DO DIA (Correção para incluir hoje na lógica) ---
  const getDayStatus = (molde: MoldeEstatistica, dateString: string) => {
    ...
    if (registro.status === 'pendente') {
      ...
      // Se é pendente e a data é menor que hoje, é 'missed' (perdeu)
      // Se é hoje, ainda é 'empty' (ou pode ser uma cor de alerta, mas deixarei empty por enquanto)
